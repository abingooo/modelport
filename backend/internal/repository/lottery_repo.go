...
func (r *lotteryRepository) DrawScheduled(ctx context.Context, campaignID int64, triggeredBy *int64, now time.Time, random service.LotteryRandomSource) (*service.LotteryDrawResult, error) {
	...
	rows, err := tx.QueryContext(ctx, `SELECT e.id,e.user_id,e.idempotency_key,
	(u.status='active' AND u.deleted_at IS NULL) AS eligible
	FROM lottery_entries e JOIN users u ON u.id=e.user_id
	WHERE e.campaign_id=$1 AND e.status='pending' ORDER BY e.id FOR UPDATE OF e,u`, campaignID)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&entry.id, &entry.userID, &entry.key, &entry.eligible); err != nil {
			_ = rows.Close()
			return nil, err
		}
	...
	assignments, err := service.AllocateScheduledLotteryPrizes(prizes, eligibleCount, random)
	if err != nil {
	...
	for _, pendingEntry := range pending {
		var prize *service.LotteryPrize
		if pendingEntry.eligible {
			prize = assignments[assignmentIndex]
			assignmentIndex++
		}
		status := service.LotteryEntryNotWon
		if prize != nil {
...
WHERE id=$1 AND is_enabled=TRUE AND awarded_count<inventory`, prize.ID)
	...
	if prize != nil && prize.PrizeType == service.LotteryPrizeBalance {
		result, err := tx.ExecContext(ctx, `UPDATE users SET balance=balance+$1,updated_at=NOW()
WHERE id=$2 AND status='active' AND deleted_at IS NULL`, prize.BalanceAmount, userID)
		if err != nil {
			return nil, fmt.Errorf("credit scheduled lottery balance: %w", err)
		}
		rows, err := result.RowsAffected()
		if err != nil {
			return nil, fmt.Errorf("credit scheduled lottery balance rows: %w", err)
		}
		if rows != 1 {
			return nil, service.ErrLotteryIneligible
		}
