...
func TestMigration233IsIdempotentAndEnforcesLotteryInvariants(t *testing.T) {
	tx := testTx(t)
	ctx := context.Background()
	_, err := tx.ExecContext(ctx, `
CREATE SCHEMA migration_233_test;
SET LOCAL search_path TO migration_233_test;
...
`)
	...
	migration, err := dbmigrations.FS.ReadFile("233_modelport_lottery_bridge.sql")
	require.NoError(t, err)
	_, err = tx.ExecContext(ctx, string(migration))
	require.NoError(t, err)
	_, err = tx.ExecContext(ctx, string(migration))
	require.NoError(t, err, "migration 233 should be idempotent")
