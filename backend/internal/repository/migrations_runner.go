...
func applyMigrationsFS(ctx context.Context, db *sql.DB, fsys fs.FS) error {
	...
	if hasModelPortBridgeMigration(fsys) {
		if err := prepareLegacyModelPortMigrations(ctx, lockConn); err != nil {
...
func prepareLegacyModelPortMigrations(ctx context.Context, db migrationConnection) error {
	var applied bool
	if err := db.QueryRowContext(ctx, `
		SELECT EXISTS (
			SELECT 1 FROM schema_migrations WHERE filename = $1
		)
	`, legacyModelPortInstructionAuditIndexesMigration).Scan(&applied); err != nil {
		return fmt.Errorf("check legacy migration %s: %w", legacyModelPortInstructionAuditIndexesMigration, err)
	}
	if !applied {
		return nil
	}
