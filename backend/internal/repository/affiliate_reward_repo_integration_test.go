...
WHERE id = $1`, reviewID)
	...
	_, err = client.ExecContext(ctx, string(migrationSQL))
	require.NoError(t, err)
	defaultInviterMigrationSQL, err := migrations.FS.ReadFile("196_affiliate_default_inviter.sql")
	require.NoError(t, err)
	_, err = client.ExecContext(ctx, string(defaultInviterMigrationSQL))
	require.NoError(t, err)
	_, err = client.ExecContext(ctx, string(defaultInviterMigrationSQL))
	require.NoError(t, err)
...
WHERE tgname IN (
	...
	require.True(t, config.Registration.DefaultInviterEnabled)
	require.Equal(t, int64(1), config.Registration.DefaultInviterUserID)
