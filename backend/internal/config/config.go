...
// DefaultGrokVoiceHTTPTimeoutSeconds bounds detached Grok Voice HTTP requests,
// including the response-body read after upstream headers arrive.
const DefaultGrokVoiceHTTPTimeoutSeconds = 300

// MaxGrokVoiceHTTPTimeoutSeconds leaves one minute of the ten-minute Custom
// Voice reservation lifetime for ownership commit and cleanup work.
const MaxGrokVoiceHTTPTimeoutSeconds = 540
...
// GatewayGrokConfig holds Grok-specific gateway behavior and scheduling knobs.
//
...
type GatewayGrokConfig struct {
	...
	// VoiceHTTPTimeoutSeconds is the total deadline for detached TTS, STT, and
	// Custom Voice HTTP requests, including response-body reads.
	VoiceHTTPTimeoutSeconds int `mapstructure:"voice_http_timeout_seconds"`
}
...
func setDefaults() {
	...
	viper.SetDefault("gateway.grok.voice_http_timeout_seconds", DefaultGrokVoiceHTTPTimeoutSeconds)
...
func (c *Config) Validate() error {
	...
	if c.Gateway.Grok.VoiceHTTPTimeoutSeconds < 1 || c.Gateway.Grok.VoiceHTTPTimeoutSeconds > MaxGrokVoiceHTTPTimeoutSeconds {
		return fmt.Errorf("gateway.grok.voice_http_timeout_seconds must be between 1-%d seconds", MaxGrokVoiceHTTPTimeoutSeconds)
	}
