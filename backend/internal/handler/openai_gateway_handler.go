...
type openAIWSTurnDownstreamBodySnapshot struct {
	turn int
	body []byte
}
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	downstreamBody := instructionAuditBody
	instructionAuditExcluded := isOpenAIInstructionAuditExcluded(c, false)
	...
	if requestPlatform == service.PlatformGrok &&
		service.GrokResponsesRequestHasSearchIntent(body) &&
		!service.GroupHasExplicitGrokSearchPrice(apiKey.Group) {
		rejectUnpricedGrokGatewayCapability(c, "Web Search")
		return
	}
	...
	for {
		...
		h.recordCyberPolicyIfMarked(c, apiKey, account, subscription, reqModel, err != nil, cyberBlockKeyHTTP, clientRequestedUsageFields(c, channelMapping, reqModel, ""), service.HashUsageRequestPayload(body), downstreamBody)
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	body, downstreamBody, err := readLenientJSONRequestBodyWithAuditSource(c.Request, h.cfg)
	if err != nil {
		if maxErr, ok := extractMaxBytesError(err); ok {
	...
	requestPlatform := openAICompatibleRequestPlatform(c.Request.Context(), apiKey)
	if requestPlatform == service.PlatformGrok &&
		service.GrokMessagesRequestHasSearchIntent(body) &&
		!service.GroupHasExplicitGrokSearchPrice(apiKey.Group) {
		rejectUnpricedGrokGatewayCapabilityAnthropic(c, "Web Search")
		return
	}
	...
	for {
		...
		h.recordCyberPolicyIfMarked(c, apiKey, account, subscription, reqModel, err != nil, cyberBlockKeyMsg, clientRequestedUsageFields(c, channelMappingMsg, reqModel, ""), service.HashUsageRequestPayload(body), downstreamBody)
		forwardDurationMs := time.Since(forwardStart).Milliseconds()
		upstreamLatencyMs, _ := getContextInt64(c, service.OpsUpstreamLatencyMsKey)
		responseLatencyMs := forwardDurationMs
		if upstreamLatencyMs > 0 && forwardDurationMs > upstreamLatencyMs {
...
func (h *OpenAIGatewayHandler) acquireResponsesUserSlot(
	...
	return h.acquireResponsesUserSlotWithLifecycle(c, userID, userConcurrency, reqStream, streamStarted, reqLog, true)
}

func (h *OpenAIGatewayHandler) acquireResponsesUserSlotForDetachedUpstream(
	c *gin.Context,
	userID int64,
	userConcurrency int,
	reqStream bool,
	...
	return h.acquireResponsesUserSlotWithLifecycle(c, userID, userConcurrency, reqStream, streamStarted, reqLog, false)
}

func (h *OpenAIGatewayHandler) acquireResponsesUserSlotWithLifecycle(
	c *gin.Context,
	userID int64,
	userConcurrency int,
	reqStream bool,
	...
	return wrapResponsesSlotRelease(ctx, userReleaseFunc, releaseOnRequestDone), true
}

func wrapResponsesSlotRelease(ctx context.Context, releaseFunc func(), releaseOnRequestDone bool) func() {
	if releaseOnRequestDone {
		return wrapReleaseOnDone(ctx, releaseFunc)
	}
	return wrapReleaseOnDone(context.WithoutCancel(ctx), releaseFunc)
}
...
func (h *OpenAIGatewayHandler) acquireResponsesAccountSlot(
	...
	return h.acquireResponsesAccountSlotWithLifecycle(c, groupID, sessionHash, selection, reqStream, streamStarted, reqLog, true)
}

func (h *OpenAIGatewayHandler) acquireResponsesAccountSlotForDetachedUpstream(
	c *gin.Context,
	groupID *int64,
	sessionHash string,
	selection *service.AccountSelectionResult,
	...
	return h.acquireResponsesAccountSlotWithLifecycle(c, groupID, sessionHash, selection, reqStream, streamStarted, reqLog, false)
}

func (h *OpenAIGatewayHandler) acquireResponsesAccountSlotWithLifecycle(
	c *gin.Context,
	groupID *int64,
	sessionHash string,
	selection *service.AccountSelectionResult,
	...
	if selection.Acquired {
		...
		return wrapResponsesSlotRelease(ctx, selection.ReleaseFunc, releaseOnRequestDone), openAISlotAcquireOK
	}
	...
	if fastAcquired {
		...
		return wrapResponsesSlotRelease(ctx, fastReleaseFunc, releaseOnRequestDone), openAISlotAcquireOK
	}
	...
	return wrapResponsesSlotRelease(ctx, accountReleaseFunc, releaseOnRequestDone), openAISlotAcquireOK
}
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	for {
		...
		var turnDownstreamBody atomic.Pointer[openAIWSTurnDownstreamBodySnapshot]
		turnDownstreamBody.Store(&openAIWSTurnDownstreamBodySnapshot{turn: 1, body: firstMessage})
		...
		hooks := &service.OpenAIWSIngressHooks{
			...
			BeforeInstructionRequest: func(turn int, payload []byte, originalModel string) error {
				turnDownstreamBody.Store(&openAIWSTurnDownstreamBodySnapshot{turn: turn, body: payload})
			...
			AfterTurn: func(turn int, result *service.OpenAIForwardResult, turnErr error) {
				...
				if snapshot := turnDownstreamBody.Load(); snapshot != nil && snapshot.turn == turn {
					downstreamBody = snapshot.body
				}
				h.recordCyberPolicyIfMarked(c, apiKey, account, subscription, turnRequestedModel, turnErr != nil, cyberBlockKey, turnUsageFields, requestPayloadHash, downstreamBody)
				turnDownstreamBody.Store(nil)
				if service.GetOpsCyberPolicy(c) != nil {
...
func (h *OpenAIGatewayHandler) recordCyberPolicyIfMarked(c *gin.Context, apiKey *service.APIKey, account *service.Account, subscription *service.UserSubscription, model string, forwardErrored bool, cyberBlockKey string, channelFields service.ChannelUsageFields, requestPayloadHash string, downstreamBody []byte) {
	...
	downstreamBody = bytes.Clone(downstreamBody)
