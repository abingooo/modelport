...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	if isGrokVideoCreateEndpoint(endpoint) && !service.GroupHasExplicitGrokVideoPrice(apiKey.Group, requestModel, requestInfo.Resolution) {
		rejectUnpricedGrokGatewayCapability(c, "Video")
		return
	}
	if endpoint.IsVideoLookupRequest() && !service.GroupHasAnyExplicitGrokVideoPrice(apiKey.Group) {
		rejectUnpricedGrokGatewayCapability(c, "Video")
		return
	}
