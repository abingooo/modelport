...
func TestSettingHandler_GetPublicSettings_ExposesProductAndUpstreamVersions(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, "0.1.183.1", resp.Data.Version)
	require.Equal(t, "0.1.183", resp.Data.UpstreamVersion)
}
