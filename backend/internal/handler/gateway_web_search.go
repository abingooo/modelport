...
func (h *GatewayHandler) WebSearch(c *gin.Context) {
	...
	if !service.GroupHasExplicitGrokSearchPrice(apiKey.Group) {
	...
	streamStarted := false
	userRelease, err := h.concurrencyHelper.AcquireUserSlotWithWait(
		c, subject.UserID, subject.Concurrency, false, &streamStarted,
	)
	if err != nil {
		reqLog.Warn("gateway.web_search.user_slot_acquire_failed", zap.Error(err))
	...
	for attempt := 0; attempt < 4; attempt++ {
		...
		if isXSearch {
			nativeResp, providerName, searchErr = h.doGrokNativeXSearch(c.Request.Context(), c, account, req, searchModel, maxResults)
		} else {
			nativeResp, providerName, searchErr = h.doGrokNativeWebSearch(c.Request.Context(), c, account, req.Query, maxResults, searchModel)
		}
		if searchErr == nil {
			break
		}
		var failoverErr *service.UpstreamFailoverError
		if !errors.As(searchErr, &failoverErr) || !failoverErr.ShouldRetryNextAccount() {
			break
		}
	...
	if searchErr != nil || nativeResp == nil {
