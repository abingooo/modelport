...
func TestOpenAIGatewayServiceRecordUsage_ResponseModelCannotBypassFallbackChannelPricing(t *testing.T) {
		...
		listAllFn: func(context.Context) ([]Channel, error) {
			return []Channel{{
				...
				ModelPricing: []ChannelModelPricing{{
					...
					Models:      []string{openAIPriceyFixtureModel},
	...
	group := &Group{ID: groupID, Platform: PlatformOpenAI, RateMultiplier: 1, LongContextPricingEnabled: true}

	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:             "openai_response_model_fallback_channel_guard",
		...
		ChannelUsageFields: ChannelUsageFields{
			ChannelID:          1,
			OriginalModel:      "unpriced-public-alias",
			BillingModelSource: BillingModelSourceResponse,
		},
	})
	...
	require.Equal(t, openAIPriceyFixtureModel, *usageRepo.lastLog.BillingModel)
	wantCost := (20*channelInputPrice + 10*channelOutputPrice) * group.RateMultiplier
	require.InDelta(t, wantCost, usageRepo.lastLog.ActualCost, 1e-12)
	require.InDelta(t, wantCost, userRepo.lastAmount, 1e-12)
}
