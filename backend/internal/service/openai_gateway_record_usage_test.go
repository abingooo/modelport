...
func TestOpenAIGatewayServiceRecordUsage_UsesRequestedModelAndUpstreamModelMetadataFields(t *testing.T) {
	...
	require.Equal(t, "gpt-5.1-codex", *usageRepo.lastLog.BillingModel)
...
func TestOpenAIGatewayServiceRecordUsage_BillsMappedRequestsUsingRequestedModel(t *testing.T) {
	...
	require.Equal(t, "gpt-5.1", usageRepo.lastLog.Model)
	require.NotNil(t, usageRepo.lastLog.BillingModel)
	require.Equal(t, "gpt-5.1", *usageRepo.lastLog.BillingModel)
...
func TestOpenAIGatewayServiceRecordUsage_ResponsesMappedBillingModelHonorsBillingModelSource(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NotNil(t, usageRepo.lastLog.BillingModel)
			require.Equal(t, tt.wantBillingModel, *usageRepo.lastLog.BillingModel)
...
func TestGatewayServiceCalculateRecordUsageCost_GroupVideoPricingDoesNotPriceImage(t *testing.T) {
	groupID := int64(130)
	groupImagePrice1K := 0.021
	groupVideoPrice := 0.75
	...
	cost := svc.calculateRecordUsageCost(
		context.Background(),
		&ForwardResult{Model: "gemini-image", ImageCount: 2, ImageSize: ImageBillingSize1K},
		&APIKey{
			GroupID: i64p(groupID),
			Group: &Group{
				ID:           groupID,
				Platform:     PlatformGemini,
				ImagePrice1K: &groupImagePrice1K,
				ModelPricing: []ChannelModelPricing{{
					Platform: PlatformGemini, Models: []string{"gemini-image"},
					BillingMode: BillingModeVideo, PerRequestPrice: &groupVideoPrice,
				}},
			},
		},
		"gemini-image",
...
func TestGatewayServiceCalculateRecordUsageCost_ChannelVideoPricingDoesNotPriceImage(t *testing.T) {
	groupID := int64(131)
	videoPrice := 0.75
	cache := newEmptyChannelCache()
	cache.pricingByGroupModel[channelModelKey{groupID: groupID, model: "gemini-image"}] = &ChannelModelPricing{
		BillingMode: BillingModeVideo, PerRequestPrice: &videoPrice,
	}
	cache.channelByGroupID[groupID] = &Channel{ID: groupID, Status: StatusActive}
	cache.groupPlatform[groupID] = ""
	cache.loadedAt = time.Now()
	...
	require.NotNil(t, cost)
	require.Equal(t, string(BillingModeImage), cost.BillingMode)
	require.InDelta(t, 2*defaultImageGenerationPrice, cost.TotalCost, 1e-12)
	require.InDelta(t, 2*defaultImageGenerationPrice, cost.ActualCost, 1e-12)
}
