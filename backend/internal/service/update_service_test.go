...
func TestUpdateServicePerformBinaryUpdateRejectsIncompleteFallbackCache(t *testing.T) {
	t.Setenv(modelPortUpdateModeEnv, updateModeBinary)
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "cached update metadata is incomplete")
}
...
func TestUpdateServiceDockerRollbackRequiresProtocolAssets(t *testing.T) {
	...
	releases := []*GitHubRelease{
		{
			TagName: "custom-v0.1.164.6",
			Assets: []GitHubAsset{
				{Name: dockerManifestAssetName},
				{Name: dockerRevisionAssetName},
			},
		},
		{
			TagName: "custom-v0.1.164.5",
			Assets:  []GitHubAsset{{Name: dockerRevisionAssetName}},
		},
		{
			TagName: "custom-v0.1.164.4",
			Assets:  []GitHubAsset{{Name: dockerManifestAssetName}},
		},
		{
			TagName: "custom-v0.1.164.3",
			Assets: []GitHubAsset{
	...
	require.Equal(t, []RollbackVersion{{
		Version: "0.1.164.6",
		HTMLURL: modelPortReleaseURL("custom-v0.1.164.6"),
	}}, versions)
	require.NoError(t, svc.RollbackToVersion(context.Background(), "0.1.164.6"))
	request, err := os.ReadFile(requestFile)
	require.NoError(t, err)
	require.Equal(t, "0.1.164.6\n", string(request))

	for _, version := range []string{"0.1.164.5", "0.1.164.4", "0.1.164.3", "0.1.164.2"} {
		err := svc.RollbackToVersion(context.Background(), version)
		require.ErrorIs(t, err, ErrRollbackVersionNotAllowed, "version %s should be rejected", version)
	}
}
