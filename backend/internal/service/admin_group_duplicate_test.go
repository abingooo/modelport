...
func cloneGroupForDuplicateTest(group *Group) *Group {
	...
	cloned.VideoModelPrices = cloneGroupVideoModelPrices(group.VideoModelPrices)
	cloned.WebSearchPricePerCall = cloneGroupValuePointer(group.WebSearchPricePerCall)
	cloned.SearchPricePer1k = cloneGroupValuePointer(group.SearchPricePer1k)
...
func TestDuplicateGroupCopiesConfigurationDeeplyAndResetsRuntimeState(t *testing.T) {
	...
	require.Equal(t, source.IsFree, duplicate.IsFree)
	...
	require.Equal(t, source.SearchPricePer1k, duplicate.SearchPricePer1k)
