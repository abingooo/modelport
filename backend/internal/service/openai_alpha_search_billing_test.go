...
func TestCalculateOpenAIRecordUsageCostWebSearchPerCall(t *testing.T) {
	...
	cost, _, err := svc.calculateOpenAIRecordUsageCostWithModel(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 3.0, 1.0, 1.0, 2.0, UsageTokens{}, "", false)
	...
	cost, _, err = svc.calculateOpenAIRecordUsageCostWithModel(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, 1.0, UsageTokens{}, "", false)
	require.NoError(t, err)
	...
	_, _, err = svc.calculateOpenAIRecordUsageCostWithModel(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, 1.0, UsageTokens{InputTokens: 10}, "", false)
	require.Error(t, err)
}
