...
const (
	...
	lotterySchedulerInterval    = 30 * time.Second
	lotterySchedulerListTimeout = 5 * time.Second
	lotterySchedulerDrawTimeout = 25 * time.Second
	lotteryCacheTimeout         = 5 * time.Second
...
func (s *LotteryService) drawDueCampaigns() {
	s.drawDueCampaignsWithTimeouts(lotterySchedulerListTimeout, lotterySchedulerDrawTimeout)
}

func (s *LotteryService) drawDueCampaignsWithTimeouts(listTimeout, drawTimeout time.Duration) {
	listCtx, listCancel := context.WithTimeout(context.Background(), listTimeout)
...
func (s *LotteryService) invalidateBalance(userID int64) {
	if s.authCacheInvalidator != nil {
		cacheCtx, cancel := context.WithTimeout(context.Background(), lotteryCacheTimeout)
