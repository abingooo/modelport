...
type SecurityNotificationRepository interface {
	Enqueue(context.Context, SecurityNotificationAudienceInput) error
	ReclaimStale(context.Context) error
	Claim(context.Context) (*SecurityNotificationOutboxItem, error)
	MarkRecipientSent(context.Context, int64, string) error
	MarkSent(context.Context, SecurityNotificationOutboxItem) error
	MarkFailed(context.Context, SecurityNotificationOutboxItem, error, time.Duration) error
}
