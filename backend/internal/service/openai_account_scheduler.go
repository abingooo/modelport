...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	decision := OpenAIAccountScheduleDecision{}
	hardSticky := grokVoiceHardAccountAffinityFromContext(ctx) && strings.TrimSpace(sessionHash) != ""
	scheduler := s.getOpenAIAccountScheduler(ctx)
	if scheduler == nil && hardSticky {
		// Permanent Grok Voice ownership must not degrade to legacy failover just
		// because the optional advanced scheduler setting is disabled.
	...
	if hardSticky && stickyAccountID <= 0 {
		return nil, decision, ErrNoAvailableAccounts
	}
