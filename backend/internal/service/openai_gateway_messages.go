...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	if account.IsDedicatedOpenAICompatibleProvider() || (account.Type == AccountTypeAPIKey && !openai_compat.ShouldUseResponsesAPI(account.Extra)) {
	...
	if account.Platform == PlatformGrok {
		...
		if priceErr := requireExplicitGrokSearchPriceForResponsesBody(c, responsesBody); priceErr != nil {
			writeAnthropicError(c, http.StatusNotFound, "not_found_error", priceErr.Error())
			return nil, priceErr
		}
	}
