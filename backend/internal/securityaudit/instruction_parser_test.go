...
func TestStrictInstructionParserBodyBoundaries(t *testing.T) {
	...
	for _, sizeMiB := range []int{1, 16, 32, 42, 64} {
		t.Run(fmt.Sprintf("%dMiB", sizeMiB), func(t *testing.T) {
			...
			// This is a body-size boundary test, not a parser benchmark. Leave
			// enough headroom for the 64 MiB case under race instrumentation.
			limits.ParseTimeout = 30 * time.Second
