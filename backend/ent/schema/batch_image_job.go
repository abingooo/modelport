...
func (BatchImageJob) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int64("group_id").Optional().Nillable().Comment("Submission-time group snapshot for usage attribution and account statistics pricing"),
		...
		field.Bool("is_free_billing").Default(false).Comment("Submission-time snapshot of the group's free-billing setting"),
