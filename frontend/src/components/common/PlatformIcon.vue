<template>
  ...
  <span
    v-else-if="providerPreset"
    :class="providerIconClass"
    :title="providerPreset.name"
    aria-hidden="true"
  >
    {{ providerShortLabel }}
  </span>
...
import { getOpenAICompatibleProviderPreset } from '@/utils/providerPresets'
...
const providerPreset = computed(() => getOpenAICompatibleProviderPreset(props.platform))
const providerShortLabel = computed(() => providerPreset.value?.shortName.slice(0, 2) ?? '')
...
const providerIconClass = computed(() => `${sizeClass.value} inline-flex items-center justify-center rounded-sm bg-current/10 font-mono text-[7px] font-bold leading-none`)
</script>
