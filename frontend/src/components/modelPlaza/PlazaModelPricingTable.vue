    ...
    <table class="w-full min-w-[860px] table-fixed border-collapse text-sm tabular-nums">
      ...
      <tbody>
        ...
        >
          ...
          >
            <span v-if="isFree" class="font-bold text-cyan-600 dark:text-cyan-300">
...
const isFree = computed(() => props.isFree === true)
const effectiveRate = computed(() => isFree.value ? 0 : (props.userRateMultiplier ?? props.rateMultiplier))
const hasCustomRate = computed(
  () => !isFree.value && props.userRateMultiplier != null && props.userRateMultiplier !== props.rateMultiplier
)
...
function paidPerMillion(value: number | null | undefined): string {
  ...
  if (isFree.value) return t('modelPlaza.badges.free')
