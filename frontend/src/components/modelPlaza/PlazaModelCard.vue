<template>
  ...
  >
    ...
    <div class="flex flex-1 flex-col px-4 py-4 sm:px-5">
      <div class="mb-4 flex min-h-10 items-center gap-3">
        <span class="flex-shrink-0 text-xs font-medium text-gray-500 dark:text-dark-400">
          {{ t('modelPlaza.card.group') }}
        </span>
        <Select
          v-if="card.offers.length > 1"
          :model-value="selectedGroupId"
          :options="groupOptions"
          class="min-w-0 flex-1"
          @update:model-value="selectGroup"
        />
        <span v-else class="min-w-0 truncate text-sm font-semibold text-gray-800 dark:text-gray-200">
          {{ currentGroup.name }}
        </span>
      </div>

      <template v-if="displayPricing">
        <div
        ...
        >
          ...
          >
            ...
            <p class="mt-1 min-h-4 truncate text-[10px] text-gray-400 dark:text-dark-500" :title="price.officialTitle">
              {{ price.officialLabel }}
            </p>
          </div>
        </div>
      ...
      <div v-if="currentGroup.description || peakNote" class="mt-auto pt-4">
        <p v-if="currentGroup.description" class="text-xs leading-5 text-gray-500 dark:text-dark-400">
...
const selectedGroupId = ref(props.card.offers[0]?.group.id ?? 0)
...
watch(
  ...
  (groupIds) => {
    if (!groupIds.includes(selectedGroupId.value)) selectedGroupId.value = groupIds[0] ?? 0
...
const billingMode = computed(() => plazaBillingMode(currentModel.value) as BillingMode)
...
const groupOptions = computed<SelectOption[]>(() =>
  props.card.offers.map(({ group }) => ({ value: group.id, label: group.name }))
)

const billingModeLabel = computed(() => t(`modelPlaza.billingModes.${billingMode.value}`))
const isPeakRateActive = computed(
  () => !currentGroup.value.is_free && currentGroup.value.applied_peak_multiplier !== 1
)

const peakNote = computed(() => {
  if (currentGroup.value.is_free || !hasPeakRate(currentGroup.value)) return ''
...
const tokenIntervals = computed(() => displayPricing.value?.intervals ?? [])

const tokenPriceCells = computed(() =>
  intervalPriceFields.value.map((field) => {
    const officialLabel = officialPriceLabel(field.key, currentModel.value.official_pricing)
    return {
      key: field.key,
      label: field.label,
      current: currentTokenPrice(field.key),
      officialLabel,
      officialTitle: officialLabel
    }
  })
)
...
function selectGroup(value: string | number | boolean | null) {
  if (typeof value === 'number') selectedGroupId.value = value
}
...
function currentPrice(value: number | null | undefined): string {
  ...
  if (currentGroup.value.is_free || value === 0) return t('modelPlaza.badges.free')
  return formatScaled(value, billingMode.value === BILLING_MODE_TOKEN ? 1_000_000 : 1, 2)
}

function officialPriceLabel(key: TokenPriceKey, pricing: PlazaOfficialPricing | null): string {
  if (!pricing || pricing[key] == null) return t('modelPlaza.card.noOfficialReference')
  const base = `${t('modelPlaza.card.officialShort')} ${formatScaled(pricing[key], 1_000_000, 2)}`
  if (key === 'cache_write_price' && pricing.cache_write_1h_price != null) {
    return `${base} · 1h ${formatScaled(pricing.cache_write_1h_price, 1_000_000, 2)}`
  }
  return base
}
