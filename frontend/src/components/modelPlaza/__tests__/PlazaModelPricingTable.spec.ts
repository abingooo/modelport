...
function mountTable(models: PlazaModel[], rateMultiplier: number, userRateMultiplier?: number | null, isFree = false) {
  return mount(PlazaModelPricingTable, {
    props: { models, rateMultiplier, userRateMultiplier: userRateMultiplier ?? null, isFree }
  })
}
...
describe('PlazaModelPricingTable', () => {
  ...
  it('免费分组展示免费且不把用户专属倍率应用到价格', () => {
    const wrapper = mountTable([tokenModel()], 1, 0.8, true)
    const text = wrapper.text()
