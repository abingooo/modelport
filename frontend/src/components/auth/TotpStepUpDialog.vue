<template>
  <div v-if="controller.visible.value" class="fixed inset-0 z-[60] overflow-y-auto">
    <div class="flex min-h-full items-center justify-center p-3 sm:p-4">
      ...
      <div
        class="relative w-full max-w-md transform rounded-xl bg-white p-4 shadow-xl transition-all dark:bg-dark-800 sm:p-6"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div class="mb-6">
          ...
          />
          <div class="grid w-full grid-cols-6 gap-1 sm:gap-2" data-test="totp-code-grid">
            <input
              v-for="(_, index) in 6"
              :key="index"
              ...
              pattern="[0-9]"
              autocomplete="off"
              class="h-11 w-full min-w-0 rounded-lg border border-gray-300 px-0 text-center text-lg font-semibold focus:border-primary-500 focus:ring-primary-500 dark:border-dark-600 dark:bg-dark-700 sm:h-12"
              :aria-label="t('stepUp.digitLabel', { position: index + 1 })"
              :disabled="verifying"
