<template>
  <div class="flex items-start gap-2 rounded border p-2"
    ...
    <!-- Per-request / Image mode: generic tier label + price -->
    <template v-else>
      <div class="min-w-0 flex-1">
        ...
        <input :value="interval.tier_label" @input="emitField('tier_label', ($event.target as HTMLInputElement).value)"
          type="text" class="input mt-0.5 text-xs" :placeholder="mode === 'image' ? '1K / 2K / 4K' : t('admin.channels.form.tierPlaceholder')" />
      </div>
      <div class="w-40">
