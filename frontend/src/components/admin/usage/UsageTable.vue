      ...
      >
        ...
        <template #cell-model="{ row }">
          <div class="space-y-0.5 text-xs">
            ...
            <div
              v-if="row.billing_model && !displayedModelNames(row).includes(row.billing_model)"
              class="break-all text-amber-700 dark:text-amber-300"
            >
...
const displayedModelNames = (row: AdminUsageLog): string[] => {
  if (row.model_mapping_chain?.includes('→')) {
    return row.model_mapping_chain.split('→').map((model) => model.trim())
  }
  return [row.model, row.upstream_model]
