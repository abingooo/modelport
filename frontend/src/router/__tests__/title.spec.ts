...
describe('resolveRouteDocumentTitle', () => {
  it('为 ModelPort 首页使用双语品牌标题且不影响自定义站点', () => {
    const route = {
      name: 'Home',
      params: {},
      meta: {
    ...
    expect(resolveRouteDocumentTitle(route, 'ModelPort')).toBe(
      '模型港 ModelPort - 统一模型网关'
    )
    expect(resolveRouteDocumentTitle(route, 'Acme Gateway')).toBe('Home - Acme Gateway')
  })
