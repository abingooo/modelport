...
async function loadCampaigns() {
  ...
  campaignError.value = ''
  try {
    const response = await lotteryAPI.list(
      { page: campaignPagination.value.page, page_size: campaignPagination.value.page_size },
      controller.signal,
    )
    if (controller.signal.aborted || campaignController !== controller) return
    ...
    if (!controller.signal.aborted && campaignController === controller) {
      campaignError.value = extractI18nErrorMessage(error, t, 'lottery.errors', t('lottery.loadFailed'))
    }
  } finally {
    if (campaignController === controller) campaignLoading.value = false
  }
}
...
async function loadHistory() {
  ...
  try {
    const response = await lotteryAPI.history(
      { page: historyPagination.value.page, page_size: historyPagination.value.page_size },
      controller.signal,
    )
    if (controller.signal.aborted || historyController !== controller) return
    history.value = response.items
    historyPagination.value = { page: response.page, page_size: response.page_size, total: response.total }
  } catch (error: unknown) {
    if (!controller.signal.aborted && historyController === controller) {
      historyError.value = extractI18nErrorMessage(error, t, 'lottery.errors', t('lottery.loadFailed'))
    }
  } finally {
