...
describe('LotteryView', () => {
  ...
  it('ignores a stale campaign response after a newer refresh finishes', async () => {
    ...
    latest.resolve(response([{ ...campaign, id: 8, name: 'Latest campaign' }]))
    await flushPromises()
    stale.resolve(response([{ ...campaign, name: 'Stale campaign' }]))
    await flushPromises()
