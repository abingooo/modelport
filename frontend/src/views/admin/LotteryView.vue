...
async function loadCampaigns() {
  ...
  try {
    const response = await lotteryAPI.admin.list({ page: pagination.page, page_size: pagination.page_size, search: filters.appliedSearch, mode: filters.mode, status: filters.status }, listController.signal)
    campaigns.value = response.items; pagination.page = response.page; pagination.page_size = response.page_size; pagination.total = response.total
  } catch (error: unknown) {
    if (!listController.signal.aborted) errorMessage.value = extractI18nErrorMessage(error, t, 'lottery.admin.errors', t('lottery.loadFailed'))
  } finally { if (!listController.signal.aborted) loading.value = false }
}
...
async function loadEntries() {
  ...
  entryController?.abort(); entryController = new AbortController(); entriesLoading.value = true; entriesError.value = ''
  try {
    const response = await lotteryAPI.admin.entries(campaign.id, { page: entryPagination.page, page_size: entryPagination.page_size }, entryController.signal)
    entries.value = response.items; entryPagination.page = response.page; entryPagination.page_size = response.page_size; entryPagination.total = response.total
  } catch (error: unknown) { if (!entryController.signal.aborted) entriesError.value = extractI18nErrorMessage(error, t, 'lottery.admin.errors', t('common.error')) }
  finally { if (!entryController.signal.aborted) entriesLoading.value = false }
}
