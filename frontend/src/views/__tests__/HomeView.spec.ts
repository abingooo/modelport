...
describe('HomeView graphic harbor', () => {
  beforeEach(() => {
    ...
    stores.app.cachedPublicSettings.site_logo = ''
  ...
  it('keeps provider cargo in the harbor without rendering a separate icon current', async () => {
    ...
    expect(wrapper.get('[data-testid="harbor"]').attributes('data-provider-count')).toBe('9')
    expect(wrapper.find('.model-current').exists()).toBe(false)
  ...
  it('honors a custom logo even when the configured site name remains ModelPort', async () => {
    stores.app.cachedPublicSettings.site_logo = '/custom-logo.png'
    const wrapper = mountView()
    await flushPromises()

    expect(wrapper.get('h1').text()).toBe('ModelPort')
    expect(wrapper.find('.hero-title img').exists()).toBe(false)
    expect(wrapper.find('.chinese-wordmark').exists()).toBe(false)
  })
